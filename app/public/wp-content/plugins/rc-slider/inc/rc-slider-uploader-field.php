...
function multi_media_uploader_field($name, $value = '', $tag_options = array('title' => 'h3', 'subtitle' => 'h4', 'content' => 'p')) {
	$image      = '">' . esc_html__( 'Add Images', 'rc-slider' ) . '';
	$image_str  = '';
	$image_size = 'full';
	$display    = 'none';
	$value      = explode( ',', $value );
	...
	$html .= '<a href="#" class="wc_multi_upload_image_button button' . $image . '">' . esc_html__('Add Images', 'rc-slider') . '</a>';
	$html .= '<input type="hidden" class="attachments-ids ' . $name . '" name="' . $name . '" id="' . $name . '" value="' . esc_attr(implode(',', $value)) . '" />';
	$html .= '<a href="#" class="wc_multi_remove_image_button button" style="display:inline-block;display:' . $display . '">' . esc_html__('Remove All Images', 'rc-slider') . '</a>';
