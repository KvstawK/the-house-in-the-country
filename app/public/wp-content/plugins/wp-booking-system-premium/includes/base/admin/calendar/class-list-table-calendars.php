...
Class WPBS_WP_List_Table_Calendars extends WPBS_WP_List_Table {
	...
	public function __construct() {
		...
		$this->set_pagination_args( array(
            'total_items' => wpbs_get_calendars( array( 'status' => 'active', 'number' => -1, 'search'  => ( ! empty( $_GET['s'] ) ? esc_attr( $_GET['s'] ) : '' ) ), true ),
	...
	private function set_table_data() {
		...
		foreach( $calendars as $calendar ) {
			...
			// Unread Bookings
			$row_data['bookings_count'] = wpbs_get_bookings(array('calendar_id' => $calendar->get('id'), 'is_read' => 0), true);

			// Pending Bookings
			$row_data['bookings_count_pending'] = wpbs_get_bookings(array('calendar_id' => $calendar->get('id'), 'status' => array('pending')), true);

			// Accepted Bookings
			$row_data['bookings_count_accepted'] = wpbs_get_bookings(array('calendar_id' => $calendar->get('id'), 'status' => array('accepted')), true);

			// Deleted Bookings
			$row_data['bookings_count_deleted'] = wpbs_get_bookings(array('calendar_id' => $calendar->get('id'), 'status' => array('trash')), true);
	...
	public function column_name( $item ) {
		...
		if( $item['status'] == 'active' ) {
			...
			$actions = array(
				...
				'duplicate_calendar' => '<a onclick="return confirm( \'' . __( "This will duplicate the calendar and copy legend items, events, availability, prices and inventory. It will not copy bookings or any iCalendar feeds.", "wp-booking-system" ) . ' \' )" href="' . wp_nonce_url( add_query_arg( array( 'page' => 'wpbs-calendars', 'wpbs_action' => 'duplicate_calendar', 'calendar_id' => $item['id'] ) , admin_url( 'admin.php' ) ), 'wpbs_duplicate_calendar', 'wpbs_token' ) . '">' . __( 'Duplicate Calendar', 'wp-booking-system' ) . '</a>',
